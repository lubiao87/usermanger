...
exports.getlogin=(req,res)=>{
    ...
    dbhelp.findOne("account",{"username":req.query.username,"pwd":req.query.pwd},(err,data)=>{
        if(data){
            ...
            res.send('恭喜用户:'+req.query.username);
        }else{
...
exports.register=(req,res)=>{
    fs.readFile(path.join(__dirname,'../view/register.html'),(error,data)=>{
        //读取到页面
        //通知浏览器将数据转换成html格式文件
        res.setHeader('content-type','text/html;charset=utf-8')
        res.send(data)
...
exports.index=(req,res)=>{
    fs.readFile(path.join(__dirname,'../view/index.html'),(error,data)=>{
        //读取到页面
        //通知浏览器将数据转换成html格式文件
        res.setHeader('content-type','text/html;charset=utf-8')
        res.send(data)
